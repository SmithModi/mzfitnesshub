...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 font-serif ${isTransparentBg
        ? 'bg-transparent py-4'
        : isBMIScrolled
          ? 'bg-white/40 backdrop-blur-md shadow-sm py-2'
          : 'bg-white/35 backdrop-blur-sm shadow-sm py-2'
      }`}>
            ...
            <div className="ml-10 flex items-center space-x-10">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className={`relative px-3 py-2 text-sm font-medium transition-all duration-200 ${useWhiteText
              ...
              <Link to="/contact" className={`ml-4 px-6 py-2.5 rounded-full font-semibold transition-all duration-300 transform hover:scale-105 ${isTransparentBg
